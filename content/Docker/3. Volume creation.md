---
title: 3. Volume creation
draft: false
tags:
---
 
```
docker volume create \
    --driver local \
    --opt type=nfs \
    --opt o=addr=172.19.90.3,rw,nfsvers=4 \
    --opt device=:/mnt/data/proxmox/video/tv \
    tv

docker volume create \
    --driver local \
    --opt type=nfs \
    --opt o=addr=172.19.90.3,rw,nfsvers=4 \
    --opt device=:/mnt/data/proxmox/video/movies \
    movies

docker volume create \
    --driver local \
    --opt type=nfs \
    --opt o=addr=172.19.90.3,rw,nfsvers=4 \
    --opt device=:/mnt/data/proxmox/downloads \
    downloads

docker volume create \
    --driver local \
    --opt type=nfs \
    --opt o=addr=172.19.90.3,rw,nfsvers=4 \
    --opt device=:/mnt/data/proxmox/roms \
    roms
    
docker volume create \
    --driver local \
    --opt type=nfs \
    --opt o=addr=172.19.90.3,rw,nfsvers=4 \
    --opt device=:/mnt/data/proxmox/books \
    books
    
docker volume create \
    --driver local \
    --opt type=nfs \
    --opt o=addr=172.19.90.3,rw,nfsvers=4 \
    --opt device=:/mnt/data/proxmox/paperless/media \
    paperless_media
    
docker volume create \
    --driver local \
    --opt type=nfs \
    --opt o=addr=172.19.90.3,rw,nfsvers=4 \
    --opt device=:/mnt/data/proxmox/paperless/consume \
    paperless_consume
    
docker volume create \
    --driver local \
    --opt type=nfs \
    --opt o=addr=172.19.90.3,rw,nfsvers=4 \
    --opt device=:/mnt/data/proxmox/immich/library \
    immich_library

docker volume create \
    --driver local \
    --opt type=nfs \
    --opt o=addr=172.19.90.3,rw,nfsvers=4 \
    --opt device=:/mnt/data/proxmox/archisteamfarm \
    archisteamfarm
```
